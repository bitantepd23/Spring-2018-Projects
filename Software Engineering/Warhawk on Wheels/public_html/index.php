		...
		case 'checkLogin' :
			...
			if(isset($data) && isset($data['student_id']))
			{
				...
				if (isset($data['access_level'])) {
					...
					echo 'access_level has not been found.';
				}
			}
...
*/
		...
		case 'editrequest':
			include('views/header.php');
			include('views/sidemenu.php');
			$sql = "SELECT ride_id, ride_name, pick_up, destination, start_time, days, frequency, round_trip
				FROM `Ride`
				WHERE ride_id =:ride_id";
			$parameters = array(':ride_id'=>$_POST['ride_id']);
			//obtain data
		...
		/*	echo "<p>".$student_id."</p>";
			...
			$sql = "insert into `Student` (student_name, student_id, campus_address, student_phone, csd_services_coordinator, type_of_equipment, dvr_applicability, dvr_counselor, dvr_address, dvr_phone, dvr_fax, dvr_email, dvr_applicable_initials, academic, non_academic, non_refundable, medical_transportation, communication, cancellation, signature) values ('$student_name', '$student_id', '$campus_address', '$student_phone', '$csd_services_coordinator', '$type_of_equipment', '$dvr_applicability', '$dvr_counselor', '$dvr_address', '$dvr_phone', '$dvr_fax', '$dvr_email', '$dvr_applicable_initials', '$academic', '$non_academic', '$non_refundable', '$medical_transportation', '$communication', '$cancellation', '$signature')";
		...
		case 'studentsearch':
			include('views/header.php');
			include('views/sidemenu.php');
			if (isset($_POST['student_search_id'])) {
				$allData = getStudentAndClasses($_POST['student_search_id']);
				$data = $allData[0];
				include('views/profileview.php');
				// include('views/ride_schedule.php');
			} else {
				include('views/student_search.php');
			}
			include('views/footer.php');
			break;
